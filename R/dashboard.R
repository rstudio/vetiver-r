...
vetiver_dashboard <- function(pins = list(), display_pins = TRUE, ...) {
  ...
  rmarkdown::output_format(
    ...
    pandoc = NULL,
    base_format = rlang::inject(flexdashboard::flex_dashboard(
      !!!dashboard_dots
    ))
  )
}
...
dashboard_read_version <- function(
  board,
  name,
  version,
  ...
  if (!pins::pin_exists(board, name)) {
    rlang::abort(
      c(
        ...
        "i" = glue(
          "To knit the example vetiver monitoring dashboard, \\
...
#' @rdname vetiver_dashboard
#' @export
pin_example_kc_housing_model <- function(
